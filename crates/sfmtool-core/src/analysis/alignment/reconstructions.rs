...
//! Fitting one reconstruction's similarity transform onto another's frame.
//!
//! Where [`estimate_alignment`] and [`ransac_alignment`] fit *point sets*, this
...
//!   observations ([`ObservationSource::SiftFiles`](crate::ObservationSource))
//!   in *both* reconstructions, because the match identity is
//!   `(image, feature index)`.
//!
...
//! target's native coordinates**. A caller that displays either reconstruction
//! somewhere other than its own frame composes that in itself — for the
//! explorer, `AppState::align_node`.

use std::collections::HashMap;

use crate::analysis::alignment::{estimate_alignment, ransac_alignment, AlignmentParams};
use crate::reconstruction::point_correspondence::find_point_correspondences;
use crate::{Se3Transform, SfmrReconstruction};
...
/// here every round walks the correspondences in scalar Rust, and the explorer
/// runs this fit **synchronously on the UI thread**. 200 rounds keeps a
/// 10⁵-correspondence pair inside a frame or two while still finding the
/// dominant transform — the preliminary all-correspondence fit that sets the
/// threshold has already done most of the work.
...
impl AlignSource {
    /// What one correspondence of this kind is called, for reporting: `cameras`
    /// or `points`.
...
/// What to fit and what to fit it on.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AlignOptions {
    /// Which correspondences to gather.
...
/// that would not converge — so the caller can leave the source where it was
/// and report it.
...
/// A flat `[x0, y0, z0, x1, …]` source/target pair, the layout the point-set
/// estimators take.
...
///
/// A repeated name on either side pairs once, first occurrence winning.
pub fn shared_images(source: &SfmrReconstruction, target: &SfmrReconstruction) -> Vec<(u32, u32)> {
