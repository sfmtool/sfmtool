...
fn classify_leaves_well_conditioned_points_finite() {
    ...
    assert_eq!(classified.point_set.infinity_point_count, n_inf);
}
...
fn classify_detects_far_point_as_infinity() {
    ...
    recon.point_set.points[0].position = Point3::new(0.0, 0.0, 10_000.0);
    recon.point_set.points[0].error = 0.5;
    ...
    assert!((classified.point_set.points[0].position.coords.norm() - 1.0).abs() < 1e-9);
    // normal is zeroed for a point at infinity.
    assert_eq!(classified.point_set.points[0].normal, Vector3::zeros());
}
...
fn classify_uses_per_point_reprojection_error() {
    ...
    let mut recon = SfmrReconstruction::demo(1);
    recon.point_set.points[0].position = Point3::new(0.0, 0.0, 300.0);

    recon.point_set.points[0].error = 0.5;
    let with_floor = recon.classify_points_at_infinity(DEFAULT_NOISE_FLOOR_PX);
    assert!(
        !with_floor.point_set.points[0].is_at_infinity(),
        "z-score above the cutoff at the 1 px floor — point stays finite"
    );
    assert_eq!(with_floor.point_set.infinity_point_count, 0);

    recon.point_set.points[0].error = 20.0;
    let with_error = recon.classify_points_at_infinity(DEFAULT_NOISE_FLOOR_PX);
    assert!(
        with_error.point_set.points[0].is_at_infinity(),
        "the point's own 20 px error inflates σ, dropping the z-score below the cutoff"
    );
    assert_eq!(with_error.point_set.infinity_point_count, 1);
}
...
fn classify_idempotent_on_infinity_points() {
    ...
    assert_eq!(
        classified.point_set.infinity_point_count,
        classified.point_set.points.len()
    );
    for pt in &classified.point_set.points {
...
fn classify_rescales_patch_frame_to_angular_extents() {
    ...
    let u = get(&classified.point_set.patch_u_halfvec_xyz);
    let v = get(&classified.point_set.patch_v_halfvec_xyz);
...
fn classify_then_materialize_preserves_apparent_patch_size() {
    ...
    let ua = round.point_set.patch_u_halfvec_xyz.as_ref().unwrap();
    let va = round.point_set.patch_v_halfvec_xyz.as_ref().unwrap();
