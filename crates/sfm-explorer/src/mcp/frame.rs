...
//! command's effect is in the very frame the agent's request woke.
//! [`App::resolve_mcp_deferred`] runs last, after the present, and answers the
//! one tool whose reply is a picture of that frame.
//!
//! These are `impl App` and live here rather than in [`crate::app`] so the
//! whole surface — including the part that has to know about `wgpu` — stays in
//! one feature-gated module.
...
impl App {
    ...
    pub(crate) fn drain_mcp(&mut self) {
        let Some(rx) = self.mcp_rx.as_mut() else {
            return;
        };
