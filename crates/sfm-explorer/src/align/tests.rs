...
//!
//! The fixture is the one the spec asks for: node B is node A put through a
//! **known** similarity, so a correct fit is the one that maps B's coordinates
//! back onto A's — and the assertion can be stated on points rather than on the
//! transform's components, which sidesteps the quaternion sign ambiguity
//! entirely.
...
fn aligning_by_cameras_recovers_a_known_similarity() {
    ...
    let fit = fit_alignment(&b, &a, options(AlignSource::Cameras, true)).expect("a fit");
...
fn aligning_by_points_recovers_a_known_similarity() {
    ...
    let fit = fit_alignment(&b, &a, options(AlignSource::Points, true)).expect("a fit");
...
fn a_rigid_fit_leaves_the_scale_at_one() {
    ...
    let fit = fit_alignment(&b, &a, options(AlignSource::Cameras, false)).expect("a fit");
...
fn two_reconstructions_with_no_shared_images_cannot_be_aligned() {
    ...
    for source in [AlignSource::Cameras, AlignSource::Points] {
        let err = fit_alignment(&b, &a, options(source, true)).expect_err("no correspondences");
        assert!(
...
fn too_few_shared_cameras_is_reported_rather_than_fitted() {
    ...
    let err = fit_alignment(&b, &a, options(AlignSource::Cameras, true)).expect_err("too few");
...
fn the_point_mode_refuses_a_reconstruction_without_feature_indexes() {
    ...
    let err = fit_alignment(&b, &a, options(AlignSource::Points, true)).expect_err("no indexes");
    assert!(err.contains("feature indexes"), "{err}");

    // The camera mode is unaffected: it never looks at observations.
    assert!(fit_alignment(&b, &a, options(AlignSource::Cameras, true)).is_ok());
}
...
fn a_pair_sharing_images_but_no_3d_points_is_reported() {
    ...
    let err = fit_alignment(&b, &a, options(AlignSource::Points, true)).expect_err("no matches");
