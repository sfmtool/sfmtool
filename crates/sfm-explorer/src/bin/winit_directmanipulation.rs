...
//! Minimal DirectManipulation example using a winit window.
//!
//! Demonstrates the three workarounds needed for DirectManipulation to work
//! with winit on Windows:
//!
//! 1. Create the DM manager BEFORE winit's EventLoop (otherwise DM never
//!    generates DM_POINTERHITTEST).
//! 2. Use SetWindowSubclass to intercept DM_POINTERHITTEST, which is
//!    delivered via SendMessage and bypasses winit's message queue.
...
mod platform {
    ...
    // WM_POINTER message constants for diagnostics
    ...
    static POINTER_MSG_COUNT: AtomicU32 = AtomicU32::new(0);
    ...
    // --- Subclass procedure (intercepts DM_POINTERHITTEST before winit's wndproc) ---
    ...
    unsafe extern "system" fn dm_subclass_proc(
        ...
        // Log pointer messages for comparison with eframe binary
        let is_pointer_msg = matches!(
            msg,
            DM_POINTERHITTEST
                | WM_POINTERUPDATE
                | WM_POINTERDOWN
        ...
        if is_pointer_msg {
            let ptr_count = POINTER_MSG_COUNT.fetch_add(1, Ordering::Relaxed);
            ...
            if ptr_count < 50 {
                ...
                let type_str = if unsafe { GetPointerType(pointer_id, &mut pointer_type) }.is_ok() {
                    match pointer_type.0 {
                        1 => "PT_POINTER",
                        2 => "PT_TOUCH",
                        3 => "PT_PEN",
                        4 => "PT_MOUSE",
                ...
                eprintln!(
                    "[DM] Pointer msg: {} id={} type={} (#{})",
                    msg_name, pointer_id, type_str, ptr_count
                );
            }
        }
    ...
    impl ApplicationHandler for App {
        fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            ...
            let attrs = WindowAttributes::default().with_title("Winit DirectManipulation");
            let window = event_loop
                .create_window(attrs)
                .expect("Failed to create window");

            let hwnd_raw = if let Ok(handle) = window.window_handle() {
            ...
            if let (Some(hwnd_raw), Some(early)) = (hwnd_raw, &self.early_dm) {
                ...
                // Attach DM viewport to the winit window
                unsafe {
    ...
    pub fn run() -> windows::core::Result<()> {
        println!("=== Winit DirectManipulation Example ===");
