...
|----------|--------|----------|
| [kdf-cache-hit-cost.md](kdf-cache-hit-cost.md) | [core/features/lazy-kdforest-query.md](../core/features/lazy-kdforest-query.md) | The `.kdf` cache costs O(resident entries) per **hit** — `touch` linear-searches a `VecDeque` of every resident key — which the layout benchmark measured at 33.5 us per fully-cached access. Stamp entries with a counter and pick the eviction victim by scan instead; then halve the shared layout's hit count by returning a borrow under the existing pin and grouping a leaf's reads by block. |
