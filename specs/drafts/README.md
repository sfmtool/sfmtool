...
| [add-image-to-tracks.md](add-image-to-tracks.md) | New; builds on [core/patch/keypoint-localization-consensus-basis.md](../core/patch/keypoint-localization-consensus-basis.md) | Adding a re-posed image's observations to the tracks it can see: per point, a visibility check, a search for the new keypoint against the consensus of the existing observations, a photometric verdict by a selectable rule, and the write-back; and the harness in [`scripts/add_image_to_tracks/`](../../scripts/add_image_to_tracks/) that chooses the rule. |
