...
|----------|--------|----------|
| [kdf-shared-descriptor-batching.md](kdf-shared-descriptor-batching.md) | [core/features/lazy-kdforest-query.md](../core/features/lazy-kdforest-query.md) | Closing the shared layout's resident-case query gap, which the layout benchmark traced to a cache-mutex acquisition and a heap allocation per checked descriptor rather than to storage: return a borrow under the existing pin, and group a leaf's reads by block. |
