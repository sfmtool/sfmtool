...
|----------|--------|----------|
| [kdf-shared-descriptor-reads.md](kdf-shared-descriptor-reads.md) | [core/features/lazy-kdforest-query.md](../core/features/lazy-kdforest-query.md) | The shared layout resolves descriptors one at a time, taking roughly twice tree-local's cache-hit count for identical work — the one cell where tree-local is still faster. Return a borrow under the existing pin, and group a leaf's reads by block. |
