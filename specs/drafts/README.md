...
| [switch-camera-model.md](switch-camera-model.md) | [gui/camera-intrinsics.md](../gui/camera-intrinsics.md), [gui/mcp-server.md](../gui/mcp-server.md), [core/reconstruction/switch-camera-model.md](../core/reconstruction/switch-camera-model.md) | Switching a camera to another camera model from the viewer: a proposal in the Camera Intrinsics panel compared against the current model there and in the Image Detail overlay, applied as one edit, and the two MCP tools that open and apply it; the Kerry Park fisheyes as the motivating case, and freeing the spline in bundle adjustment as the companion change. |
