...
| [track-at-pixel.md](track-at-pixel.md) | New; builds on [core/bench/editable-track.md](../core/bench/editable-track.md); its cascade is filed as [core/bench/track-at-pixel.md](../core/bench/track-at-pixel.md) | One operation that builds a high-quality track centred on a pixel of one image, or refuses naming the stage that failed and what it measured; the evidence it may draw on; the leave-one-track-out harness in [`scripts/track_at_pixel/`](../../scripts/track_at_pixel/README.md) that judges candidate algorithms against ground truth; and what is still open about which algorithm the operation keeps. |
...
| [switch-camera-model.md](switch-camera-model.md) | [gui/camera-intrinsics.md](../gui/camera-intrinsics.md), [cli/reconstruction/xform/xform-command.md](../cli/reconstruction/xform/xform-command.md), [gui/mcp-server.md](../gui/mcp-server.md) | Switching a camera to another camera model as a fit over the angles where the old model is trusted, in one core operation reached from the Camera Intrinsics panel (as a proposal compared against the current model there and in the Image Detail overlay), from MCP and from `sfm xform --camera-model`; the Kerry Park fisheyes as the motivating case, and freeing the spline in bundle adjustment as the companion change. |
