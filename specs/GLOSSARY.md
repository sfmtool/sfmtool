...
| **ghost outline** | Image Detail's display-only outline of the track-stage patch, projected into an image the track has no observation in and drawn at the ghost opacity (0.3); constant `GHOST_OPACITY`, type `Ghost` | `phantom`, `preview`, `projection` | *preview* is taken by the drag's transient track and *projection* by the patch's projected centre and the *Proj. off* column. *Ghost* is the usual word in editors for a faint stand-in that takes no input, which is both halves of what it is |
