...
| **switch camera model** | replace cameras of a reconstruction by cameras fitted to them, in the model named; a spline camera switched to its own spline model is refitted at a new coefficient count or domain. Function `reconstruction::switch_camera_model`, the MCP tool `switch_camera_model`, the viewer edit `specs/gui/edits/switch-camera-model.md`, `sfm xform --camera-model` | a separate `refit_spline` wire tool or edit | one operation covers a change of model and a change of a spline's parameterization, so every surface that does one does the other and reports it the same way. The viewer's button for the spline case says **Refit spline…**, because that is what a person asks for there |
