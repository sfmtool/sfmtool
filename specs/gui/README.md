...
| [background-operations.md](background-operations.md) | A long operation on a worker thread while the viewer stays live: what makes it safe under the document model, the Background panel under the Scene tree that shows what is running and what it has spent its time on, what the rest of the viewer may do meanwhile, and the two-level reply and read tools an agent sees. |
