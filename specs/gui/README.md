...
| [gui-scene-graph.md](gui-scene-graph.md) | *Draft.* Multi-reconstruction support: several `.sfmr` files loaded at once as scene-graph nodes, per-node visibility/tint and similarity transforms with an in-GUI "Align to…" operation, per-reconstruction GPU resource bundles, widened pick encoding, and the Scene Graph tree panel. |
