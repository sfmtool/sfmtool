...
│   │       └── state/
│   │           └── ops.rs        # AppState's reconstruction operations (load, align, resect)
...
| `state/ops.rs` | `AppState`'s reconstruction *operations*, as a second `impl` block: `load_file`, `load_demo`, `align_node`, `resect_image` and `load_resect_matches`. These are the methods that read a file off disk, run a fit or a resection, build a `SceneNode` out of the answer and write the sentence the user reads about it. |
