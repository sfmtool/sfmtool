...
the seed relaxation's ring fill-in and final re-estimation
([seed-relaxation.md](../geometry/seed-relaxation.md)), and any caller that
holds poses and a camera and wants the structure those poses imply.
...
- **Seed relaxation, ring fill-in.** `floor` at the member's angular bound,
  `cheirality` on, `bar` at the adjustment's final pixel bound, `few =
  bearing`, `marks` off. Every candidate cluster is estimated from scratch at
  the current poses and lens.
- **Seed relaxation, final re-estimation.** As the fill-in with `bar` off:
  the adjustment has already trimmed the observations the bound would cut.
...
- **Relaxation parity.** The seed relaxation's fill-in and final
  re-estimation on a stored member reproduce the previous implementation's
  positions, flags and census, with any difference confined to the last bits
  of the solve and reported with its cause.
