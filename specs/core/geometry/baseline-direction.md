...
`sfmtool._sfmtool.geometry.baseline_directions`. NOT yet adopted by the seed
relaxation; see "Adoption" below.
...
The seed relaxation ([seed-relaxation.md](seed-relaxation.md)) is the intended
caller and does NOT yet use this. Switching it over changed the relaxation's
placed centres on a colinear-camera fixture, and the cause is not this
operation: it agrees with the implementation it replaces to `1e-15` on the
direction and exactly on every other field, over 55 edges including that
fixture's own.

What the switch exposed is that the translation averaging downstream is
ill-posed when every edge of a graph carries the SAME direction, which is what a
camera moving along a straight line produces. The averaging minimizes the part
...
Fed exactly colinear directions the averaging returns arbitrary positions. The
implementation this replaces produced directions carrying rounding asymmetries
around `1e-17`, and those asymmetries, not any measurement, were what the
downstream solve was resolving the spacing from.

So the adoption waits on the averaging: a colinear graph needs to be recognized
and refused, or resolved by something other than pairwise directions. Landing
the operation first keeps the two questions apart, and keeps the reading that
found the ill-posedness.
