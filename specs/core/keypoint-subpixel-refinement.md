...
- **The representative sampler defaults to anisotropic** (independent of the
  refine `sampler` knob): the representative is a *stored texture*, so quality
  wins over the per-step refine cost, and the aniso footprint avoids bilinear
  under-sampling on foreshortened views.
