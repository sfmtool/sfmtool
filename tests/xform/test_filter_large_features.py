...
def test_remove_large_features_basic(seoul_bull_workspace_deprecated, tmp_path):
    """Test that large features filter removes some points."""
    # Use a threshold that should remove some but not all points
    max_sizes = _compute_max_feature_sizes(seoul_bull_workspace_deprecated)
...
def test_remove_large_features_lenient(seoul_bull_workspace_deprecated, tmp_path):
...
def test_remove_large_features_embedded_patches(
    seoul_bull_workspace_deprecated, tmp_path
):
