...
> _Status (2026-07-08, commit e176f74): Done — the doc was deleted. Its durable §1/§2 content (S/W conversion math, export inverse, epipolar/round-trip invariants) was promoted into `sfmr-file-format.md` § "Conversions happen at the I/O boundary"; the 7 code/spec references were repointed at the format spec; the convention contract's stable home was already the format spec's Coordinate System Conventions section (added alongside the doc). Only the non-code GUI visual check remains._
...
3. **Decide the fate of the three dead `sfm align` options** — `--max-error`, `--iterative`, and `--visualize` are accepted and documented as functional but never used (`align/multi.py:315-317`). Implement them or remove them (and update the spec either way); today users get silent no-ops.
   > _Status (2026-07-08): Done — removed all three; the "shortest path" prose fix rolls into #1/#5's spec sweep. See the per-spec finding above._
