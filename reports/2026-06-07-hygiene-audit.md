...
> _Status (2026-06-09): Done — two commits. First, extracted a pure `_flag_frame(...) -> list[str]` in `recon_discontinuity.py`, now the single source of truth for per-frame thresholding; both the console table and `report.py` call it, so the two can no longer drift. Removed three dead accumulator dicts (`step_edge_flags`/`cov_edge_flags`/`obs_frame_flags`, written but never read) and fixed a latent divergence: `report.py` had fed `_f()`-sanitized values to its flag recompute, so an infinite `overlap_drop` (total covisibility break) silently dropped the "Cov" flag in JSON — it now flags raw values, matching the console and the spec's "always flags" rule (motion-command.md L297-298). Second, peeled the two console-render passes (per-frame table + end-of-run summary) into a new `motion/_recon_console.py` (`print_frame_table` / `print_summary`), and extracted `_aggregate_flagged_edges(...)` (step 3) and `_select_core_edges(...)` (step 4) as pure helpers. `analyze_reconstruction` is now compute + orchestration only — no inline formatting — and shrank from ~515 to ~210 lines; the `reproj_errors` computation moved into the compute phase so `print_summary` is pure presentation. The import cycle (`_recon_console` imports the pure helpers from `recon_discontinuity`) is broken with a lazy import in `analyze_reconstruction`. Console output and JSON are byte-identical; full Python suite (1318 tests) passes, ruff clean (this commit)._
...
> _Status (2026-06-09): Done — moved the whole ~500-line body into `recon_clone.rs::clone_with_changes(inner, py, kwargs)`; the `#[pymethods]` method is now a thin 4-line wrapper, shrinking `py_sfmr_reconstruction.rs` 1304→814 lines. The two near-identical `extract_array1!`/`extract_array2!` macros collapsed into one shared `extract_ndarray!` base (40-line body → one copy) with two tiny 1D/2D wrappers, preserving the exact error messages and ergonomic call sites. Deviated from the proposal in two ways, deliberately: (a) kept the logic as a free function rather than a `CloneChanges` struct + `from_kwargs` — the per-field validation is interleaved with the running recon state (e.g. "colors length must match point count" only after a `positions` resize), so a pure parse-then-apply struct would have to re-thread that state and add a `Option` field per kwarg for no clarity gain; (b) kept the macros in `recon_clone.rs` rather than `helpers.rs` — there is no cross-module consumer and the messages are clone-specific. Clippy clean; `maturin develop --release` rebuilt; the `test_sfmr_reconstruction_rust_bindings.py` suite passes (this commit)._
...
> _Status (2026-06-09): Done — extracted the ~300-line `camrig cp` block (the command is `sfm camrig cp`, not `cam cp` as the snapshot guessed) into `tests/test_camrig_cp.py` (317 lines); `test_camrig.py` 1015→712. Moved the four cross-command CLI tests (`test_solve_uses_camera_config` + the three `--camera-model` rejections across solve/match/to-colmap-db) and their shared `_seoul_bull_native_intrinsics` helper into `tests/test_camera_config_cli.py` (199 lines); `test_camera_config.py` 766→580 and its docstring refreshed to the current `camera/config.py`+`camera/setup.py` module names. Consolidated the CLI tests into one file rather than scattering across three command-test modules, to keep the shared fixture helper in one place. The optional resolver/pattern split (`test_camrig_resolve.py`) was left as-is. All 98 tests across the four touched modules pass; ruff clean (this commit)._
