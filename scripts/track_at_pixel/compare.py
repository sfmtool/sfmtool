...
by ``(point, image)``; the counts over the queries every run holds are printed
as well, so runs over different samples are not mistaken for each other.
"""
...
def main(argv=None) -> int:
    ...
    for run, rows in data.items():
        ...
        common_good = sum(1 for k in common if rows[k].get("good"))
        print(
            f"{run.name[:34]:34s} {len(rows):7d} {len(ok):6d} {len(good):6d} "
